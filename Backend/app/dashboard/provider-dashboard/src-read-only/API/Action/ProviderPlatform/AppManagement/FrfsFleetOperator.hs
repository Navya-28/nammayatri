...
type PostFrfsFleetOperatorQueryRow =
  ( ApiAuth
      ...
      ('DSL)
      (('PROVIDER_APP_MANAGEMENT) / ('API.Types.Dashboard.AppManagement.FRFS_FLEET_OPERATOR) / ('API.Types.Dashboard.AppManagement.FrfsFleetOperator.POST_FRFS_FLEET_OPERATOR_QUERY_ROW))
      :> API.Types.Dashboard.AppManagement.FrfsFleetOperator.PostFrfsFleetOperatorQueryRow
  )
...
postFrfsFleetOperatorQueryRow :: (Kernel.Types.Id.ShortId Domain.Types.Merchant.Merchant -> Kernel.Types.Beckn.Context.City -> ApiTokenInfo -> BecknV2.OnDemand.Enums.VehicleCategory -> Lib.GtfsDataServer.Types.NandiTable -> Lib.GtfsDataServer.Types.QueryBody -> Environment.FlowHandler [Lib.GtfsDataServer.Types.NandiRow])
postFrfsFleetOperatorQueryRow merchantShortId opCity apiTokenInfo vehicleCategory table req = withFlowHandlerAPI' $ Domain.Action.ProviderPlatform.AppManagement.FrfsFleetOperator.postFrfsFleetOperatorQueryRow merchantShortId opCity apiTokenInfo vehicleCategory table req
