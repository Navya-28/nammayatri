      ...
      then pure False
        ...
        if val /= Just messageId && counterCancellationPossible
          then do
            ...
            -- Refund only if the booking ever reached CONFIRMED, since that is where OnConfirm
            -- debits. A cancel arriving for a booking that never confirmed must not be credited a
            -- trip it never spent, and that is reachable: OnConfirm's expiry path and
            -- onConfirmFailure both fire a Technical CONFIRM_CANCEL on bookings they have just
            -- marked FAILED, and the on_cancel lands here.
            --
            -- Stated as the pre-confirm statuses rather than a CONFIRMED allow-list, because the
            -- booking has usually moved on by the time the callback arrives: OnCancel/Core sets
            -- CANCEL_INITIATED and OnStatus sets COUNTER_CANCELLED, both from CONFIRMED and both
            -- still owing the rider a trip. An allow-list silently dropped those refunds.
            when (currentStatus `notElem` [DFRFSTicketBooking.NEW, DFRFSTicketBooking.APPROVED, DFRFSTicketBooking.PAYMENT_PENDING, DFRFSTicketBooking.CONFIRMING, DFRFSTicketBooking.FAILED]) $
