...
startJourney riderId confirmElements forcedBookedLegOrder journey mbEnableOffer mbIsMockPayment = do
  ...
  void $
    withTryCatch "startJourney:confirmPassCoveredLegs" $ do
      ...
      --
      -- Non-FRFS legs have to be counted from the plans too, not from legBookings: a Taxi leg has
      -- no FRFSTicketBooking row, so findBySearchId cannot see it and its fare would be invisible
      -- here. Walk legs are excluded by bookingAllowed, which leaves Taxi -- the one mode that
      -- costs money and settles outside FRFS.
      let isFrfsMode mode = mode `elem` [DTrip.Bus, DTrip.Metro, DTrip.Subway]
          journeyHasPayableLeg =
            any (not . FRFSPassOverride.isFullyPassCovered . (.overriddenAmount)) legBookings
              ...
              || any (\plan -> plan.legInfo.bookingAllowed && not (isFrfsMode plan.legInfo.travelMode)) plans
