...
select2' (personId, merchantId) estimateId req mbJourneyLegData = withPersonIdLogTag personId $ do
  ...
  journeyID <- Redis.whenWithLockRedisAndReturnValue (selectEstimateLockKey personId) 60 $ do
    -- Expiry guard for tip re-selects: must run before cancelSearchUtil so an
    -- expired estimate is rejected without cancelling the rider's ongoing search.
    -- Skipped when journey-leg data is present (multimodal legs).
