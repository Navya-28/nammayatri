...
postWebhook mbSig rawBody = withLogTag "MetaWebhook" $ do
  globalEnabled <- asks (.metaBotEnabled)
  -- Global kill-switch stays first and cheap (in-memory bool, no DB/Redis
  -- round trip) — still works even if the DB is having a bad day. ANDed with
  -- the per-row `enabled` below.
  if not globalEnabled
    then pure Success
    else case peekPhoneNumberId rawBody of
      Nothing -> do
      ...
      Just phoneNumberId ->
        CQMWC.findByPhoneNumberId phoneNumberId >>= \case
          Nothing -> do
            ...
            pure Success
          Just cfg
            | not cfg.enabled -> pure Success -- per-row kill-switch: same "DROP, not pause" semantics as the global one
