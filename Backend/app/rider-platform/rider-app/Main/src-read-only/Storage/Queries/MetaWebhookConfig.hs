...
findAllEnabled :: (EsqDBFlow m r, MonadFlow m, CacheFlow m r) => (Kernel.Prelude.Bool -> m ([Domain.Types.MetaWebhookConfig.MetaWebhookConfig]))
findAllEnabled enabled = do findAllWithKV [Se.Is Beam.enabled $ Se.Eq enabled]
