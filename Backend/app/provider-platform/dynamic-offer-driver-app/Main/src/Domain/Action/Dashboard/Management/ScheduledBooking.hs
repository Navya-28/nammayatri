...
postScheduledBookingUnassign merchantShortId opCity transactionId _mbRequestorId = do
  ...
  -- unassign = cancel the current ride + force-reallocate: recreates an unassigned NEW booking
  -- under the same transactionId and re-enters scheduled auto-dispatch (ops can also reassign).
  let bookingCReason =
