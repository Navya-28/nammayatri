...
postFinanceManagementTdsReimbursementRequestSubmit merchantShortId opCity requestorId req = do
  ...
  assessmentYear <- DTdsReqExtra.mkAssessmentYear req.assessmentYear
  ...
  Redis.withLockRedisAndReturnValue lockKey 60 $ do
    ...
    (periodStart, periodEnd) <- quarterDateRange assessmentYear quarter
    ...
    QTdsReq.create
      DTdsReq.FinanceTdsReimbursementRequest
        { id = requestId,
          ...
          tdsRate = req.tdsRate,
          documentId = req.documentId.getId,
...
quarterDateRange :: DTdsReq.AssessmentYear -> DTdsReq.Quarter -> Flow (UTCTime, UTCTime)
quarterDateRange (DTdsReq.AssessmentYear ay) quarter = do
  fyStartYear <- case T.splitOn "-" ay of
    (startText : _) | Just y <- readMaybe (T.unpack startText) -> pure (y - 1 :: Integer)
    _ -> throwError $ InvalidRequest $ "Invalid assessment year: " <> ay
  let mkDay y m d = UTCTime (DT.fromGregorian y m d) 0
  pure $ case quarter of
...
getFinanceManagementTdsReimbursementStatus merchantShortId opCity apiQuarter assessmentYearText requestorId = do
  ...
  case mbRequest of
    ...
    Just request -> do
      ...
      invoiceLines <- forM mappings buildStatusInvoiceLine
...
buildStatusInvoiceLine :: DTdsMap.FinanceTdsReimbursementInvoiceMapping -> Flow API.TdsReimbursementStatusInvoiceLine
buildStatusInvoiceLine mapping = do
  invoice <- QFinanceInvoice.findById mapping.invoiceId >>= fromMaybeM (InvalidRequest $ "Invoice not found: " <> mapping.invoiceId.getId)
  indirectTaxTxns <- QIndirectTax.findByInvoiceNumber (Just invoice.invoiceNumber)
  (baseValue, gst) <- case indirectTaxTxns of
    (txn : _) -> pure (txn.taxableValue, txn.totalGstAmount)
    _ -> throwError $ InvalidRequest $ "No indirect tax transaction found for invoice " <> invoice.invoiceNumber
...
getFinanceManagementTdsReimbursement merchantShortId opCity requestId = do
  ...
  documentUrl <- resolveTdsDocumentUrl (Id request.documentId)
