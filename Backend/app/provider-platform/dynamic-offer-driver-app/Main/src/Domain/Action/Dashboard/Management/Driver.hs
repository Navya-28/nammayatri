...
---------------------------------------------------------------------
getDriverLoginOtp :: ShortId DM.Merchant -> Context.City -> Maybe Text -> Maybe Text -> Flow Common.DriverLoginOtpRes
getDriverLoginOtp merchantShortId _ mbMobileNumber mbCountryCode = do
  merchant <- findMerchantByShortId merchantShortId
  ...
  driver <- QPerson.findByMobileNumberAndMerchantAndRole countryCode mobileNumberHash merchant.id DP.DRIVER >>= fromMaybeM (InvalidRequest "Driver not found")
  ...
  pure
    Common.DriverLoginOtpRes
      { driverId = cast driver.id,
        ...
        expiresAt = addUTCTime (fromIntegral (token.authExpiry * 60)) token.updatedAt
      }
