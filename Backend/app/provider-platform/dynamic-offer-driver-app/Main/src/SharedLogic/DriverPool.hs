...
scheduledRideFilter currentSearchInfo merchantId merchantOpCityId isRental isIntercity transporterConfig driverPoolWithActualDistResult = do
  ...
  where
    -- Case 2 short-circuit ladder: no hold -> surface (no query); single-slot holder -> exclude (== today);
    -- multi-hold -> cap + two-neighbour interval feasibility on the derived committed set
    scheduledCandidateFilter now driverInfo
      | not (isJust driverInfo.latestScheduledBooking) = return True
      | transporterConfig.maxScheduledHoldsPerDriver <= 1 = return False
      | otherwise = do
