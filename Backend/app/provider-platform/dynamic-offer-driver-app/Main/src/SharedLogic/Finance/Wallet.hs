...
recordStripeChargeLedger ctx funder amount refType
  ...
  | otherwise = do
    result <- runFinance ctx $ case funder of
      FundByPlatform -> void $ transfer BuyerAsset SellerExpense amount refType
      FundByCustomer -> do
        transfer_ SellerExpense SellerLiability amount refType
        void $ transfer BuyerAsset SellerRevenue amount refType
      FundByDriver -> do
        transfer_ SellerExpense SellerLiability amount refType
        void $ transfer OwnerLiability SellerRevenue amount refType
    pure (void result)
