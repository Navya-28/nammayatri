...
data DriverPoolDataUpdate = DriverPoolDataUpdate
  { -- Class 2 fields (sync, LTS authoritative for dispatch)
    ...
    selectedInstantAcceptTiers :: SetField [ServiceTierType],
...
emptyUpdate =
  DriverPoolDataUpdate
    { active = Unchanged,
      ...
      selectedInstantAcceptTiers = Unchanged,
      enabled = Unchanged,
      blocked = Unchanged,
...
applyUpdate now u d =
  d
    { DPD.lastUpdatedAt = Just now,
      ...
      DPD.selectedInstantAcceptTiers = applyField u.selectedInstantAcceptTiers d.selectedInstantAcceptTiers,
